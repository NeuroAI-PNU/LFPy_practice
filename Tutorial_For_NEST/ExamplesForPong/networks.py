...
class PongNetDopa(PongNet):
    ...
    def __init__(self, apply_noise=True, num_neurons=20):
        ...
        if apply_noise:
            ...
            self.poisson_noise = nest.Create(
                "poisson_generator",
                self.num_neurons,
                params={"rate": self.poisson_rate},
            )
            nest.Connect(
                self.poisson_noise,
                self.motor_neurons,
    ...
    def apply_synaptic_plasticity(self, biological_time):
        ...
        reward_current = (
            self.dopa_signal_factor * target_n_spikes / total_n_spikes
...
class PongNetRSTDP(PongNet):
    ...
    def __init__(self, apply_noise=True, num_neurons=20):
        ...
        if apply_noise:
            self.background_generator = nest.Create(
                "noise_generator", self.num_neurons, params={"std": BG_STD}
            )
            nest.Connect(
    ...
    def calculate_stdp(
        self, pre_spikes, post_spikes, only_causal=True, next_neighbor=True
    ):
        ...
        for spike, position in zip(post_spikes, positions):
            ...
            if position > 0:
                ...
                facilitation += self.stdp_amplitude * np.exp(
                    -(spike - before_spike) / self.stdp_tau
                )
            if position < len(pre_spikes):
                after_spike = pre_spikes[position]
                depression += self.stdp_amplitude * np.exp(
                    -(after_spike - spike) / self.stdp_tau
                )
